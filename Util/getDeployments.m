...
if ~isempty(strfind(ddb,'csv')), isCSV = 1; else isCSV = 0; end
...
deployments = func('DeploymentData', 'EndFieldTrip', fId);

% query the ddb for all sites related to these deployments
lenDep = length(deployments);
for i=1:lenDep
    if i==1
        sites = func('Sites', 'Site', deployments(i).Site);
    else
        sites(i) = func('Sites', 'Site', deployments(i).Site);
    end
end
